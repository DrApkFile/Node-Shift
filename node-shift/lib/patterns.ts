...
pub mod circuit_breaker {
    ...
    /// Pauses the entire program functionality.
    /// In Web2, we'd toggle a 'feature flag' in a DB or config service.
    ...
    /// Resumes the program functionality.
    pub fn resume(ctx: Context<UpdateState>) -> Result<()> {
        let config = &mut ctx.accounts.config;
        config.is_paused = false;
        Ok(())
    }

    /// Gated instruction that checks the circuit breaker state.
    /// In Web3, every instruction must explicitly check this config account.
    pub fn sensitive_operation(ctx: Context<SensitiveOperation>) -> Result<()> {
...
pub struct UpdateState<'info> {
    ...
    #[account(
        ...
        seeds = [b"config"],
        bump,
        has_one = admin // Only the admin can pause/resume
    )]
...
#[error_code]
pub enum ErrorCode {
    #[msg("The program is currently paused for maintenance or emergency.")]
    ProgramPaused,
}` },
...
pub fn process_instruction(
    ...
    Admin -->|2. Call Pause()| Program[Circuit Program]
